...
- **Fixed Registration Validation Bug During Migration** (August 6, 2025): Resolved critical issue where manual verification codes like "OUGKCG" were not being found during validation. Updated `getEventRegistrationByUniqueId` function to search in all possible code storage locations: `uniqueId`, `registrationData.manualVerificationCode`, and root-level `manualVerificationCode`. Registration validation now works correctly for all code types and storage formats.
