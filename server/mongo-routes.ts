...
export function registerMongoRoutes(app: Express) {
  ...
  // Get customer payment history by email
  app.get("/api/payments/customer/:email", async (req: Request, res: Response) => {
    try {
      ...
      const tickets = await mongoStorage.getTickets({ ownerEmail: email });
      
      // Get all registrations for this customer
      const registrations = await mongoStorage.getEventRegistrations(null, { email });
      ...
      for (const ticket of tickets) {
        if (ticket.paymentStatus === 'paid') {
          ...
          payments.push({
            id: ticket._id.toString(),
            type: 'ticket',
            reference: ticket.paymentReference || 'Manual Payment',
            amount: ticket.price,
            currency: ticket.currency,
            status: 'paid',
      ...
      for (const registration of registrations) {
        if (registration.paymentStatus === 'paid' || registration.paymentStatus === 'completed') {
          ...
          payments.push({
            id: registration._id.toString(),
            type: 'registration',
            reference: registration.paymentReference || 'Manual Payment',
